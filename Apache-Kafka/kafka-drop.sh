...
case "$1" in
  start)
        ...
        nohup $JAVA_HOME/bin/java -jar $KAFDROP_TOOL_EXPORT_PATH/kafdrop-4.0.1.jar --kafka.brokerconnect=$KAFKA_HOST --server.port=$PORT --management.server.port=$PORT &> /dev/null &
        ;;
  stop)
        # Stop daemons.
